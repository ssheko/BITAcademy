...
static AFX_EXTENSION_MODULE ImplicitDllDemoDLL = { false, nullptr };

// 외부 애플리케이션이 실행될 때 로드되는 부분.
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ImplicitDllDemo.DLL을 초기화하고 있습니다.\n");

		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(ImplicitDllDemoDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ImplicitDllDemoDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ImplicitDllDemo.DLL을 종료하고 있습니다.\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(ImplicitDllDemoDLL);
	}
	return 1;   // 확인
}
