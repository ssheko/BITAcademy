...
std::vector<SOCKETINFO*> userList;
CRITICAL_SECTION cs;	// UserList 동기화를 위함.
...
unsigned int WINAPI WorkerThread(void* pParam)
{
	...
	while (1) {
		...
		for (int i = 0; i < userList.size(); i++) {
			if (client_sock == userList[i]->sock)
				currentClient = i;
		}
		...
		if (ptr->recvbytes > ptr->sendbytes) {
			...
			//int errorFlag = 0;
				...
				for (int i = 0; i < userList.size(); i++)
				{
					if (i == currentClient)
						continue;
					send(userList[i]->sock, ptr->wsabuf.buf, strlen(ptr->wsabuf.buf) + 1, 0);
				}
				retval = WSASend(userList[currentClient]->sock, &(ptr->wsabuf), 1, &sendbytes,
