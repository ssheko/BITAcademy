...
LRESULT CALLBACK ChildProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		SendMessage(hParent, WM_CTLCOLORBTN, (WPARAM)hwnd, (LPARAM)hdc);
	...
	LPSTR	  lpCmdLine, int nShowCmd)
{
	...
	wc.lpszClassName = TEXT("First");	// 구분 key가 되는 값
	wc.lpszMenuName = 0;
	wc.style = 0;
	...
	wc.lpszClassName = TEXT("CHILD");	// class 이름이 다르니까 가능.

	RegisterClass(&wc); // 자식용 윈도우 클래스 등록
	//=========================================

	// 3. 윈도우 창 만들기 // 부모 윈도우 클래스 이름으로 부모 윈도우 생성
	HWND hwnd = CreateWindowEx(0, TEXT("first"), TEXT("Hello"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, 0, 0, hInst, 0);			// 4. 윈도우 보여주기
