...
// 리턴 : 성공여부
/*----------------*////////////////////////*----------------*/
bool NetClient::Disconnect(INT64 clientID)
{
...
// 리턴 : (bool) 성공여부
/*----------------*////////////////////////*----------------*/
bool NetClient::SendPacket(INT64 clientID, StreamBuffer * packet)
{
	//int index = FindIndex(clientID);
		...
		if (retval == SOCKET_ERROR)
		{
			...
			if (errorCode != ERROR_IO_PENDING)
			{
				if (errorCode == WSAENOBUFS)
					LOG(L"Client", LOG_ERROR, L"SessionID : %d , WSAENOBUFS", session->clientID);
				else if (errorCode == WSAECONNRESET)
					;
				else if(errorCode == WSAESHUTDOWN)
					LOG(L"Client", LOG_WARNING, L"SessionID : %d , WSAESHUTDOWN", session->clientID);
				else
				{
					printf("sendpost %d\n", errorCode);
					//session->sendFlag = FALSE;
...
// 리턴 : (int) 변환된 인덱스
/*----------------*////////////////////////*----------------*/
int NetClient::FindIndex(INT64 clientID)
{
	INT64 mask = 0xffff;
...
void NetClient::RecvPost(Session * session)
{
	...
	{
		// 종료
		LOG(L"Client", LOG_ERROR, L"SessionID : %d , RecvQueue Full", session->clientID);
		shutdown(session->socket, SD_SEND);
		return;
	}
	// wsabuf 세팅
