...
private:
	...
	Session			_sessionArray[1];			// 세션을 담을 배열
	...
	static unsigned int WINAPI ConnectThreadMain(LPVOID lpParam); // connect스레드 메인
...
public:
	...
	NetClient();
	NetClient(const char * configFile);
	virtual ~NetClient();
	...
	virtual bool OnConnectionRequest(char * ClientIP, int Port) = 0;        //< accept 직후
